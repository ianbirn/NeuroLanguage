...
void test(int N, int D, int *langAM, int *iMHV, wchar_t *itemMemory, int imSize, char langLabels[][4], int length, wchar_t *unknown) {
    ...
    double maxAngle = -1.0; double angle2 = 0.0;
    for(int l=0; l<length; l++) {
        ...
        if (angle > maxAngle) {
            ...
            sprintf(predicLang, "%s", langLabels[l]);
        }
        else if (angle > angle2) {
            angle2 = angle;
            sprintf(predicLang2, "%s", langLabels[l]);
        }
        free(tmp);
    }
    free(testSumHV);

    double difference = maxAngle - angle2;
    if(difference <= 0.05) {
        wprintf(L"Warning!! This prediction is not strong\n");
    }
